# Reacor Pipes - in-process channels and streams

Reactor Pipes is an evolvement and extension of our recent works and ideas on
[Reactor]() and [Meltdown]().

Main features include:

  * [Named Pipe topologies](https://github.com/reactor/reactor-extensions/blob/master/reactor-pipe/README.md#named-stream-topologies)
  * [Anonymous Pipe topologies](https://github.com/reactor/reactor-extensions/blob/master/reactor-pipe/README.md#anonymous-stream-topologies)
  * [Matched Lazy Pipes](https://github.com/reactor/reactor-extensions/blob/master/reactor-pipe/README.md#matched-lazy-streams)
...
Reactor Pipes is a lightweight Stream Processing system, which was developed with performance,
...
Reactor Pipes can be also used to make asynchronous/concurrent programs simpler, easier to
...
Reactor Pipes approach is simple: develop on one box, for one box, break processing in logical
...
`Pipe` is a term coming from Reactive Programming. Stream looks a little like a collection
...
`Source` / `Sink` are used to describe the order of functions in your topologies.
...
The main difference between Java streams and Reactor Pipes is the dispatch flexibility
...
As you can see, toplogies will be created per-entity, which opens op a lot of opportunities
...
intPipe.map(Key.wrap("key2"), Key.wrap("key3"), (Atom<Integer> state) -> { // Use a supplier to capture state in closure 
...
```java
Pipe<Integer> stream = new Pipe<>(firehose);
